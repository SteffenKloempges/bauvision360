...
import { ServiceData, professionalServices, craftingServices, serviceData, ServiceDataMap, sachverstaendigenServices } from '../services';
...
const SingleService = () => {
    ...
    useEffect(() => {
        if (pathname.includes("bauleitung")) {
            setDataState(professionalServices);
        } else if (pathname.includes("handwerklich")) {
            setDataState(craftingServices);
        } else if (pathname.includes("sachverstaendigung")) {
            setDataState(sachverstaendigenServices)
        }
    }, [pathname])
        ...
        <section className="px-5 py-8 lg:py-12 text-center">
            <FadeIn>
                <h1 className="text-4xl lg:text-5xl font-bold text-primary leading-tight">
                    {pathname.includes("bauleitung") ? "Koordinierende Leistungen" : pathname.includes("handwerklich") ? "Handwerkliche Leistungen" : "Sachverständigen Leistungen"}
                </h1>
                <p className="text-xl text-gray-600">
                    {pathname.includes("professional") ? "Professionelle Unterstützung für Ihr Projekt" : pathname.includes("handwerklich") ? "Handwerkliche Leistungen" : "Unparteiische Einschätzung durch Expertise"}
                </p>
            </FadeIn>
        </section>
        <section className="px-5 py-8">
            <div className={pathname.includes("bauleitung") ? "lg:grid-cols-2 max-w-7xl mx-auto grid md:grid-cols-2 gap-8 " : pathname.includes("handwerklich") ? "lg:grid-cols-4 max-w-7xl mx-auto grid md:grid-cols-2 gap-8" : "lg:grid-cols-1 max-w-2xl mx-auto grid md:grid-cols-2 gap-8 "}>
